...
The app provisions the hook on a Machine the first time it is used. This is our own tooling, not a third-party dependency, so it falls under the same exception that lets the app install its own CLI. The embedded POSIX `sh` script is installed at `~/.local/share/ai-mission-manager/hooks/ai-mission-manager-agent-state-hook.sh`. Provisioning registers a separate command for each provider event, with both the provider and its target state fixed as arguments. It drains the provider's standard input without parsing the JSON payload, so event interpretation remains in the provider configuration rather than a helper process.

For app-started Runs, the backend builds `AI_MISSION_MANAGER_STATE_FILE` from the Machine's home directory and Run id. The hook writes there; local state files live at `~/.local/state/ai-mission-manager/runs/run-<run-id>.json`. The hook uses a private umask, creates app-owned directories with mode `0700`, writes a `0600` temporary file beside the destination, and renames it into place. Existing parent directory permissions are left alone. It then best-effort sets the `@ai_mission_manager_run_state` Pane option; losing that notification does not lose the durable state. Recovery checks this location first and still reads the old database-adjacent `agent-state/` file for Runs recorded before this change. Legacy files are left where they are.

Run state is stored in the domain and SQLite as `unknown`, `working`, `blocked`, or `finished`; the app reconciles the durable state file when it opens and whenever it reconnects to a Run Pane. A blocked Run is projected into Needs Attention, while all Run-state transitions leave Item state unchanged.
